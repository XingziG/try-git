...
public abstract class Binop extends Node { // Binop class is the father of the
	...
	public void addRandomKids(OperatorFactory o, TerminalFactory t,
		...
		// choosing a random number between 0 and (numOperators + numIndepVars).
			...
			if (i < o.getNumOps()) { // If this number is less than
										// numOperators, add a new operator as
				lChild = o.getOperator(rand); // the left child (by calling
												// getOperator()).
				lChild.setDepth(this.getDepth() + 1); // Set the new child’s
														// depth to one larger
														// than n’s depth.
				lChild.addRandomKids(o, t, maxDepth, rand); // Then call the new
															// child’s
															// addRandomKids()
															// method. This is
															// the recursive
															...
															// tree building
															// process going.
			} else { // If this number is larger than numOperators, add a new
						// terminal as
				lChild = t.getTreminal(rand); // the left child (by calling
												// getTerminal()).
				lChild.setDepth(this.getDepth() + 1); // Set the new child’s
														// depth to one larger
														// than n’s depth.
