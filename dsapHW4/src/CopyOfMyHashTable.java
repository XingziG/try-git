...
public class CopyOfMyHashTable implements MyHTInterface {
	/**
	...
	/**
	 * implement constructor with no initial arrayLength The default arrayLength is
	 * 10, which is not a prime number...
	 */
	public CopyOfMyHashTable() {
	 ...
	 * 
	 * the passCapacity must not be <= 0 or bigger than the maximun value of integer
	 * 
	 * @param passCapacity
	 */
	public CopyOfMyHashTable(int passCapacity) {
	...
	public void insert(String value) {
		...
		boolean collision = false;
			...
			while (myhashArray[hashVal] != null
				...
				if (value.equals((myhashArray)[hashVal].value)) {
					myhashArray[hashVal].frequency += 1;
					return;
				}
	...
	private void rehash() {
		...
		count = 0;
		boolean collision = false;
		arrayLength = myhashArray.length * 2; //to update arrayLength = myhashArray.length * 2
		...
		DataItem[] oldhashArray = new DataItem[myhashArray.length]; //store the old hash table
		System.arraycopy(myhashArray, 0, oldhashArray, 0, myhashArray.length); // copy from the old hashtable
		...
		if(collision)
			numOfCollision++;
		System.out.println("Rehashing " + count + " items, new size is " + arrayLength);
	}
