...
public class SortedLinkedList implements MyListInterface {
	...
	private Node recRemove(Node head, int index) {
		// base 1, wrong index
		if (index < 0)
			throw new IllegalArgumentException("bad index for remove");
			...
			// base 4, wrong index
			if (head == null)
				throw new IndexOutOfBoundsException("bad index for remove");
			else
